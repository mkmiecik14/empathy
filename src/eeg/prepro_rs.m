% EEG Preprocessing Pipeline Step 1 - FOR RESTING STATE RECORDINGS
...
for iter = 1:num_iters
    ...
    try
        ...
        % Creating variables
        this_ss = NUM(iter);
        this_ss_path = dir(fullfile(data_dir, num2str(this_ss), 'as*.vhdr'));
        ...
        % Extracting first resting state block
        EEG = pop_epoch( EEG, {  'S111'  }, [0  20], 'newname', this_ss_name, 'epochinfo', 'yes');
        [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'overwrite','on','gui','off'); 
        ...
        saveas(gcf, fullfile(halloween_dir, strcat('id-', num2str(this_ss), '-rs', '.png'))); % saves as png
