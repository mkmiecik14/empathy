...
if (render) {
  f <- here::here("output", "analysis-pca-3-exp-res.rds")
  exp_res <- read_rds(file = f)

  f <- here::here("output", "analysis-pca-3-q-res.rds")
...
exp_res <- read_rds(file = "../../output/analysis-pca-3-exp-res.rds")
q_res <- read_rds(file = "../../output/analysis-pca-3-q-res.rds")
...
This document plots and interprets the results from the principal component analysis (PCA) in `src/analysis/analysis-pca-3.R`.

**UPDATE** from `analysis-pca-2.R`: shoulder_CPM was excluded and the only CPM measurement is knee_CPM.
